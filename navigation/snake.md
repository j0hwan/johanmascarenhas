---
layout: page
title: Snake
permalink: /Snake/
---


<div style="text-align: center;">
    <h2>Score: <span id="score">0</span></h2>
    <canvas id="gameCanvas" width="500" height="500" style="border: 2px solid black;"></canvas>
    <br>
    <button id="restartButton" style="font-size: 20px; padding: 10px 20px; background-color: #3498db; color: white; border: none; border-radius: 10px; cursor: pointer;">Restart Game</button>
    <div>
        <h3>Power-ups:</h3>
        <div style="display: flex; justify-content: center;">
            <div style="width: 50px; height: 50px; background-color: blue; margin: 10px;"></div>
            <div style="width: 50px; height: 50px; background-color: green; margin: 10px;"></div>
            <div style="width: 50px; height: 50px; background-color: yellow; margin: 10px;"></div>
            <div style="width: 50px; height: 50px; background-color: red; margin: 10px;"></div>
            <div style="width: 50px; height: 50px; background-color: purple; margin: 10px;"></div>
        </div>
    </div>
</div>

<script>
    const canvas = document.getElementById('gameCanvas');
    const ctx = canvas.getContext('2d');
    const box = 50; // Bigger boxes for the 10x10 grid
    let snake = [{x: 4 * box, y: 5 * box}];
    let direction = '';
    let food = {x: Math.floor(Math.random() * 10) * box, y: Math.floor(Math.random() * 10) * box};
    let powerUps = [];
    let score = 0;
    let gameSpeed = 200;
    let gameInterval;

    const snakeHead = new Image();
    snakeHead.src = 'https://howtodrawforkids.com/wp-content/uploads/2022/04/9-snake-head-drawing-lesson.jpg'; // Replace with actual image URL
    const apple = new Image();
    apple.src = 'data:image/jpeg;base64,'; // Replace with actual image URL

    const powerUpEffects = ['blue', 'green', 'yellow', 'red', 'purple'];
    const negativeEffects = ['black', 'black', 'black'];

    function randomizePowerUp() {
        const color = Math.random() > 0.7 ? negativeEffects[Math.floor(Math.random() * 3)] : powerUpEffects[Math.floor(Math.random() * 5)];
        powerUps.push({
            x: Math.floor(Math.random() * 10) * box,
            y: Math.floor(Math.random() * 10) * box,
            color: color
        });
    }

    document.addEventListener('keydown', changeDirection);
    document.getElementById('restartButton').addEventListener('click', restartGame);

    function restartGame() {
        clearInterval(gameInterval);
        snake = [{x: 4 * box, y: 5 * box}];
        direction = '';
        score = 0;
        document.getElementById('score').innerText = score;
        food = {x: Math.floor(Math.random() * 10) * box, y: Math.floor(Math.random() * 10) * box};
        powerUps = [];
        gameSpeed = 200;
        startGame();
    }

    function changeDirection(event) {
        if (event.key === 'ArrowUp' && direction !== 'DOWN') {
            direction = 'UP';
        } else if (event.key === 'ArrowDown' && direction !== 'UP') {
            direction = 'DOWN';
        } else if (event.key === 'ArrowLeft' && direction !== 'RIGHT') {
            direction = 'LEFT';
        } else if (event.key === 'ArrowRight' && direction !== 'LEFT') {
            direction = 'RIGHT';
        }
    }

    function drawFood() {
        ctx.drawImage(apple, food.x, food.y, box, box);
    }

    function drawSnake() {
        for (let i = 0; i < snake.length; i++) {
            ctx.drawImage(snakeHead, snake[i].x, snake[i].y, box, box);
        }
    }

    function drawPowerUps() {
        powerUps.forEach(powerUp => {
            ctx.fillStyle = powerUp.color;
            ctx.fillRect(powerUp.x, powerUp.y, box, box);
        });
    }

    function moveSnake() {
        let head = {...snake[0]};
        
        if (direction === 'UP') head.y -= box;
        if (direction === 'DOWN') head.y += box;
        if (direction === 'LEFT') head.x -= box;
        if (direction === 'RIGHT') head.x += box;

        if (head.x === food.x && head.y === food.y) {
            score++;
            document.getElementById('score').innerText = score;
            food = {x: Math.floor(Math.random() * 10) * box, y: Math.floor(Math.random() * 10) * box};
        } else {
            snake.pop();
        }

        snake.unshift(head);

        powerUps.forEach((powerUp, index) => {
            if (head.x === powerUp.x && head.y === powerUp.y) {
                if (powerUp.color === 'blue') {
                    gameSpeed -= gameSpeed * 0.2;
                } else if (powerUp.color === 'green') {
                    score += 10;
                } else if (powerUp.color === 'yellow') {
                    snake.push({x: snake[snake.length - 1].x, y: snake[snake.length - 1].y});
                } else if (powerUp.color === 'red') {
                    gameSpeed = 100;
                } else if (powerUp.color === 'purple') {
                    score += 50;
                } else if (powerUp.color === 'black') {
                    gameSpeed += 50;
                }
                powerUps.splice(index, 1);
            }
        });
    }

    function checkCollision() {
        let head = snake[0];
        if (head.x < 0 || head.x >= canvas.width || head.y < 0 || head.y >= canvas.height) {
            clearInterval(gameInterval);
            alert(`Game Over! Your score is: ${score}`);
        }

        for (let i = 1; i < snake.length; i++) {
            if (head.x === snake[i].x && head.y === snake[i].y) {
                clearInterval(gameInterval);
                alert(`Game Over! Your score is: ${score}`);
            }
        }
    }

    function updateGame() {
        ctx.clearRect(0, 0, canvas.width, canvas.height);
        drawFood();
        drawSnake();
        drawPowerUps();
        moveSnake();
        checkCollision();
    }

    function startGame() {
        gameInterval = setInterval(updateGame, gameSpeed);
    }

    // Randomize power-ups every 10 seconds
    setInterval(randomizePowerUp, 10000);
    
    startGame();
</script>

<style>
    canvas {
        display: block;
        margin: 20px auto;
        background-color: #f3f3f3;
    }
</style>
